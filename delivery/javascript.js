...
$(document).ready(function(){
	...
	$(document).on('click','.remove_row',function(){
		...
		var final_amount = $('#final_total_amount').text().replace("SHS","");
		...
		if(isNaN(result_amount)){
			...
			if(total_product_amount == ""){
				...
				$('#total_amount').text('SHS '+minus_total);
			}else{
				$('#final_total_amount').text('SHS 0.00');
			}
			
		}else{
			$('#final_total_amount').text('SHS '+result_amount);
		}
	...
	function final_total(count){
		...
		$('#final_total_amount').text('SHS '+final_product_amount);
	}
