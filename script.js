...
function GrandTotal(){
  ...
  $(TotalPriceArr).each(function(){
    TotalValue += parseFloat($(this).text().replace(/,/g, "").replace("SHS",""));
  });
  ...
  if(discount != null){
    ...
    $("#totalValue").text(accounting.formatMoney(f_discount,{symbol:"SHS",format: "%s %v"}));
    $("#totalValue1").text(accounting.formatMoney(TotalValue,{format: "%v"}));
  }else{
    $("#totalValue").text(accounting.formatMoney(TotalValue,{symbol:"SHS",format: "%s %v"}));
    $("#totalValue1").text(accounting.formatMoney(TotalValue,{format: "%v"}));
  }
};
        ...
        title: "Enter number of items:",
		  ...
		  .then((value) => {
            ...
            swal("Error","Please enter a number!","error");
    				...
    				$('#tableData').append("<tr class='prd'><td class='barcode text-center'>"+barcode+"</td><td class='text-center'>"+product+"</td><td class='price text-center'>"+accounting.formatMoney(price,{symbol:"SHS",format: "%s %v"})+"</td><td class='text-center'>"+unit+"</td><td class='qty text-center'>"+value+"</td><td class='totalPrice text-center'>"+accounting.formatMoney(total,{symbol:"SHS",format: "%s %v"})+"</td><td class='text-center p-1'><button class='btn btn-danger btn-sm' type='button' id='delete-row'><i class='fas fa-times-circle'></i></button><tr>");
	          GrandTotal();
        }
			}
  });
...
$(document).on('click','.Enter',function(){
  ...
  if (TotalPriceArr == 0){
    ...
    $('.price').each(function(){
      price.push($(this).text().replace(/,/g, "").replace("SHS",""));
    });
    ...
    .then((value) => {  
      if(value == "") {
        ...
        if(isNaN(qtynum)){
          ...
          //   TotalValue += parseFloat($(this).text().replace(/,/g, "").replace("SHS",""));
          // });
          var TotalValue = parseFloat($('#totalValue').text().replace(/,/g, "").replace("SHS",""));
          ...
          if(TotalValue > qtynum){
            ...
            $.ajax({
              ...
              success: function(data){
                ...
                if( data == "success"){
                  swal({
                    title: "Change is " + accounting.formatMoney(change,{symbol:"SHS",format: "%s %v"}),
                    icon: "success",
                    buttons: "Okay",
                  })
                  .then((okay)=>{
